...
export default function LoginScreen({ nav }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      let result;
      if (isDriver) {
        result = await loginDriver(id, pw);
      } else {
        result = await loginPoliceStation(id, pw);
      }
      ...
      if (msg.includes('INVALID_CREDENTIALS') || msg.includes('401')) {
        Alert.alert(
          '❌ Login Failed',
          isDriver
            ? 'Wrong Badge ID or password.\n\nMake sure you are using your BADGE ID (e.g. TX-YDE-001), not your vehicle plate.'
        ...
        Alert.alert(
          '📡 Connection Error',
          'Could not connect to server.\n\nThe server may be waking up — this takes about 50 seconds on first use.\n\nPlease wait and try again.'
        );
      } else {
            ...
            <View style={s.card}>
              ...
              <View style={s.fieldWrap}>
                ...
                <Text style={s.fieldHint}>
                  {isDriver
                    ? 'The ID you chose when registering (e.g. TX-YDE-001) — NOT your vehicle plate'
...
const s = StyleSheet.create({
  ...
  helpTitle:     { fontSize: 13, fontWeight: '800', marginBottom: 8 },
