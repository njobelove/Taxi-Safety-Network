...
import { checkAndPlayAlarm } from '../services/AlertSoundService';
...
export default function PoliceDashboard({ nav }) {
  ...
  useEffect(() => {
    loadAlerts();
    const iv = setInterval(loadAlerts, 10000); // poll every 10s for faster alarm
    return () => {
      clearInterval(iv);
  ...
  const loadAlerts = async () => {
    try {
      ...
      // Play alarm sound if new alerts arrived
      checkAndPlayAlarm(active, mutedRef.current);
  ...
  const handleResolve = async (alertId) => {
    const alertToRate = alerts.find(a => (a._id || a.id) === alertId);
  ...
  return (
    <SafeAreaView style={s.safe}>
      <RatingModal
        visible={showRating}
        alert={ratingAlert}
        onClose={() => { setShowRating(false); setRatingAlert(null); }}
        onRated={() => { setShowRating(false); setRatingAlert(null); }}
      />
