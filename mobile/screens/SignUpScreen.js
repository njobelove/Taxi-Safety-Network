...
export default function SignupScreen({ nav }) {
  ...
  const validateAndRegister = async () => {
    ...
    try {
      if (isDriver) {
        await registerDriver({ fullName, badgeId: badgeId.trim().toUpperCase(), phoneNumber, network, vehiclePlate: vehiclePlate.trim().toUpperCase(), city, password });
      } else {
        await registerPoliceStation({ stationName, stationId: stationId.trim().toUpperCase(), district, city, commanderName, emergencyLine, password });
      }
      ...
      if (msg.includes('BADGE_EXISTS') || msg.includes('STATION_EXISTS') || msg.includes('duplicate') || msg.includes('11000')) {
        Alert.alert('⚠ ID Already Taken',
          isDriver
            ? 'Badge ID "' + badgeId.toUpperCase() + '" is already registered.\n\nTry: TX-YDE-' + (Math.floor(Math.random()*900)+100)
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <View style={s.topSection}>
            ...
            <View style={s.roleRow}>
              ...
              >
                <MaterialIcons name="directions-car" size={18} color={role === 'driver' ? accent : 'rgba(255,255,255,0.8)'} />
              ...
              >
                <MaterialIcons name="local-police" size={18} color={role === 'police' ? accent : 'rgba(255,255,255,0.8)'} />
          ...
          <View style={s.formCard}>
            ...
            {isDriver ? (
              <>
                ...
                <Field label="BADGE ID *"       value={badgeId}      onChange={v => setBadgeId(v.toUpperCase())} placeholder="e.g. TX-YDE-001" auto="characters" hint="This is your login ID — must be unique. NOT your vehicle plate." />
              ...
              <>
                ...
                <Field label="STATION ID *"     value={stationId}     onChange={v => setStationId(v.toUpperCase())} placeholder="e.g. YDE-PS-001" auto="characters" hint="This is your login ID — must be unique." />
            ...
            >
              {loading ? <ActivityIndicator color="#fff" /> : (
                <>
                  ...
                  <Text style={s.submitTxt}>{isDriver ? 'CREATE DRIVER ACCOUNT' : 'CREATE POLICE ACCOUNT'}</Text>
                </>
              )}
            </TouchableOpacity>
...
const s = StyleSheet.create({
  safe:         { flex: 1 },
  topSection:   { padding: 20, paddingTop: 16, paddingBottom: 24 },
  ...
  fieldHint:    { fontSize: 10, color: '#aaa', marginBottom: 6, lineHeight: 14 },
  ...
  submitBtn:    { borderRadius: 14, paddingVertical: 16, alignItems: 'center', marginTop: 8, marginBottom: 12, flexDirection: 'row', justifyContent: 'center', gap: 8 },
