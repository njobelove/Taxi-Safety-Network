...
const OPTIONS = [
  { id: 'robbery',  icon: 'warning',          tag: 'ARMED ROBBERY',    accent: RED,       bg: '#fde8e8', title: 'ROBBERY / BRAQUAGE',   desc: 'Armed robbery or violent threat.'   },
  { id: 'assault',  icon: 'personal-injury',  tag: 'PHYSICAL THREAT',  accent: '#e65100', bg: '#fff3e0', title: 'ASSAULT / AGRESSION',   desc: 'Physical attack or harassment.'     },
  { id: 'accident', icon: 'car-crash',        tag: 'VEHICLE INCIDENT', accent: BLUE,      bg: '#e8f0fe', title: 'ACCIDENT',               desc: 'Vehicle collision or damage.'       },
  { id: 'medical',  icon: 'medical-services', tag: 'HEALTH EMERGENCY', accent: '#827717', bg: '#f5f5dc', title: 'MEDICAL EMERGENCY',      desc: 'Injury or health emergency.'        },
  { id: 'theft',    icon: 'security',         tag: 'PROPERTY CRIME',   accent: '#6a1b9a', bg: '#f3e5f5', title: 'THEFT / VOL',            desc: 'Theft of property or belongings.'   },
];
...
export default function EmergencyScreen({ nav, location }) {
  ...
  const handleConfirm = async () => {
    if (!selected) return Alert.alert('Select Alert Type', 'Please choose an incident type first.');
    setSending(true);
    await triggerSOS({ user, location, voiceUri, token, nav, alertType: selected });
    setSending(false);
  };
    ...
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={[s.locationBanner, { backgroundColor: location ? '#e8f5e9' : '#fde8e8' }]}>
          <MaterialIcons name={location ? 'gps-fixed' : 'gps-off'} size={20} color={location ? GREEN : RED} />
          <View style={{ flex: 1 }}>
            ...
            <Text style={s.locationSub}>
              {location ? location.latitude.toFixed(5) + '° N, ' + location.longitude.toFixed(5) + '° E' : 'Please allow location access'}
            </Text>
          </View>
        </View>
        ...
        {voiceUri && (
          <View style={s.voiceBanner}>
            ...
            <Text style={s.voiceBannerTxt}>Voice note ready — will play automatically on SOS</Text>
          </View>
        )}
        ...
        {OPTIONS.map(o => (
          ...
          >
            ...
            <View style={s.optBody}>
              <Text style={[s.optTag, { color: o.accent }]}>{o.tag}</Text>
        ...
        <View style={s.confirmCard}>
          <View style={s.disclaimerRow}>
            ...
            <Text style={s.disclaimerTxt}>
              ...
              {'• Alert all nearby police & drivers\n• Share your exact GPS location\n• Send SMS to Police 117'}
            </Text>
          </View>
          <View style={s.selectedRow}>
            <MaterialIcons name={selected ? 'check-circle' : 'radio-button-unchecked'} size={18} color={selected ? GREEN : '#aaa'} />
            <Text style={[s.selectedTxt, { color: selected ? GREEN : '#aaa' }]}>
              {selected ? 'Selected: ' + (OPTIONS.find(o => o.id === selected)?.title || '') : 'No incident type selected yet'}
            </Text>
          </View>
          <TouchableOpacity
          ...
          >
            {sending ? <ActivityIndicator color="#fff" size="small" /> : (
              <><MaterialIcons name="sos" size={24} color="#fff" /><Text style={s.confirmTxt}>CONFIRM SOS ALERT</Text></>
            )}
          </TouchableOpacity>
        </View>
...
const s = StyleSheet.create({
  ...
  networkBadge:   { backgroundColor: GOLD, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 5 },
  networkTxt:     { fontSize: 11, fontWeight: '800', color: '#111' },
