...
const ACFG={robbery:{icon:'warning',label:'ROBBERY',color:RED},assault:{icon:'personal-injury',label:'ASSAULT',color:'#e65100'},accident:{icon:'car-crash',label:'ACCIDENT',color:BLUE},medical:{icon:'medical-services',label:'MEDICAL',color:'#827717'},theft:{icon:'security',label:'THEFT',color:'#6a1b9a'},sos:{icon:'sos',label:'SOS',color:RED}};
...
export default function HistoryScreen({nav}){
  ...
  return(
    <SafeAreaView style={s.safe}>
      ...
      {loading?(
        <View style={s.loadBox}><ActivityIndicator size="large" color={RED}/><Text style={{color:'#888',marginTop:12}}>Loading...</Text></View>
      ):(
        <ScrollView showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>{setRefreshing(true);loadHistory();}} tintColor={RED}/>}>
          ...
          {displayed.map(alert=>{
            ...
            const cfg=ACFG[alert.alertType]||ACFG.sos;
            ...
            return(
              <View key={alertId} style={[s.alertCard,{borderLeftColor:cfg.color},isResolved&&{opacity:0.85}]}>
                ...
                <TouchableOpacity onPress={()=>lat&&Linking.openURL('https://maps.google.com?q='+lat+','+lng)} style={{flexDirection:'row',alignItems:'center',gap:4,marginBottom:8}}>
                  ...
                  <Text style={s.alertLoc}>{alert.location?.address||(lat?parseFloat(lat).toFixed(4)+'° N':'Location not recorded')}{lat?'  → tap':''}</Text>
                </TouchableOpacity>
                <View style={s.driverRow}>
                  <MaterialIcons name="person" size={16} color="#888"/>
                  <Text style={s.driverInfo}>{alert.driverName||'—'}  ·  {alert.driverId||'—'}{alert.vehiclePlate?'  ·  '+alert.vehiclePlate:''}</Text>
                </View>
              </View>
            );
          })}
...
const s=StyleSheet.create({
  ...
  errorBox:{alignItems:'center',paddingVertical:50,gap:12},
  errorTxt:{fontSize:14,color:'#888',textAlign:'center'},
  ...
  driverRow:{flexDirection:'row',alignItems:'center',gap:8,backgroundColor:'#f9f9f9',borderRadius:8,padding:8},
