...
import { View, Text, StyleSheet, TouchableOpacity, SafeAreaView, ActivityIndicator, Alert, ScrollView, Linking } from 'react-native';
...
export default function DisactivationScreen({ nav }) {
  ...
  const loadMyAlerts = async () => {
    ...
    try {
      ...
      setMyAlerts((data.alerts || []).filter(a => a.driverId === user?.badgeId && a.status !== 'resolved'));
    } catch (e) { Alert.alert('Error', 'Could not load alerts: ' + e.message); }
  ...
  const handleSafe = async (alertId) => { setBusy(true); await doDeactivate(alertId); setBusy(false); };
  const handleAll  = async () => {
    setBusy(true);
    for (const a of myAlerts) await doDeactivate(a._id || a.id);
    setBusy(false); setAllDone(true);
  };
    ...
    return (
      <SafeAreaView style={s.safe}>
        ...
        <View style={s.doneWrap}>
          ...
          <Text style={s.doneSub}>{'You confirmed you are safe.\n\n✅ Removed from all dashboards\n✅ Voice broadcast stopped\n✅ Police notified\n\nThank you for using TSN. Stay safe!'}</Text>
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      {loading ? (
        <View style={s.loadBox}><ActivityIndicator size="large" color={RED} /><Text style={{ color: '#666', marginTop: 12 }}>Loading your alerts...</Text></View>
      ) : (
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 16 }}>
          <View style={s.statusCard}>
            <MaterialIcons name="warning" size={18} color={RED} />
            <View style={{ flex: 1 }}>
              <Text style={s.statusTitle}>{myAlerts.length} ACTIVE ALERT{myAlerts.length !== 1 ? 'S' : ''}</Text>
              <Text style={s.statusSub}>Broadcasting to all police and drivers.{'\n'}Tap I AM SAFE to deactivate each alert.</Text>
            </View>
          </View>
          ...
          <View style={s.driverCard}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={s.driverMeta}>{user?.badgeId || '—'}  ·  {user?.vehiclePlate || '—'}</Text>
            </View>
          </View>
            ...
            return (
              <View key={alertId} style={[s.alertCard, { borderLeftColor: col }]}>
                ...
                <View style={s.alertLocRow}>
                  ...
                  <Text style={s.alertLoc}>{alert.location?.address || (lat ? parseFloat(lat).toFixed(4) + '° N' : 'Location not recorded')}</Text>
                </View>
                <View style={s.btnRow}>
                  {lat && (
                  ...
                  <TouchableOpacity style={s.callBtn} onPress={() => Linking.openURL('tel:117')}>
                    <MaterialIcons name="local-phone" size={14} color="#90caf9" />
                    <Text style={s.callBtnTxt}>117</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[s.safeBtn, busy && { opacity: 0.6 }]} onPress={() => handleSafe(alertId)} disabled={busy}>
                    {busy ? <ActivityIndicator size="small" color="#fff" /> : (
                      <><MaterialIcons name="check-circle" size={16} color="#fff" /><Text style={s.safeBtnTxt}>I AM SAFE</Text></>
                    )}
                  </TouchableOpacity>
                </View>
              </View>
          ...
          {myAlerts.length > 1 && (
            <TouchableOpacity style={[s.allBtn, busy && { opacity: 0.6 }]} onPress={handleAll} disabled={busy}>
              {busy ? <ActivityIndicator color="#fff" /> : (
                <><MaterialIcons name="check-circle" size={20} color="#fff" /><Text style={s.allBtnTxt}>DEACTIVATE ALL {myAlerts.length} ALERTS — I AM SAFE</Text></>
              )}
            </TouchableOpacity>
          )}
          ...
          <View style={s.emergCard}>
            ...
            <View style={s.emergRow}>
              {[['Police\n117','117',RED],['Ambulance\n15','15',BLUE],['Fire\n118','118','#7d6608']].map(([lbl,tel,col]) => (
                <TouchableOpacity key={tel} style={[s.emergBtn, { backgroundColor: col }]} onPress={() => Linking.openURL('tel:' + tel)}>
                  <MaterialIcons name="local-phone" size={18} color="#fff" />
                  <Text style={s.emergBtnTxt}>{lbl}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
...
const s = StyleSheet.create({
  ...
  dashBtn:      { backgroundColor: RED, borderRadius: 14, paddingVertical: 16, paddingHorizontal: 30, flexDirection: 'row', alignItems: 'center', gap: 8 },
  ...
  safeBtn:      { flex: 1, backgroundColor: GREEN, borderRadius: 12, paddingVertical: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6 },
  safeBtnTxt:   { fontSize: 14, fontWeight: '900', color: '#fff' },
  allBtn:       { backgroundColor: GREEN, borderRadius: 14, paddingVertical: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 12 },
